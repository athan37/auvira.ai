            ...
            <nav className="flex flex-col gap-1 px-6 pt-4">
              {NAV_LINKS.map((link, i) => (
                <motion.a
                  ...
                  className="border-b border-[#d2d2d7]/60 py-4 text-2xl font-semibold text-[#1d1d1f]"
                >
                  {link.label}
                </motion.a>
              ))}
              <div className="pt-6">
                <MarketingLink href="/auth/signin" className="w-full justify-center">
                  Sign in
                </MarketingLink>
              </div>
            </nav>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <nav className="hidden items-center gap-8 text-xs font-normal text-[#1d1d1f] sm:flex">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
