...
export async function runSingleShotStrategy(
  ...
  for (const rel of candidates) {
    try {
      const content = await fs.readFile(path.join(options.workspacePath, rel), 'utf-8');
...
For background/color requests: update BOTH src/app/globals.css (body background) AND src/app/page.tsx (change preset.pageBg and preset.surfaceBg to matching bg-[#hex] values) when those files exist.`,
