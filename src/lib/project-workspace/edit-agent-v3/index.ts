...
/**
 * Website Edit Agent V3: EditContext → plan → domain tools → verify → summarize.
 */
export async function runWebsiteEditAgentV3(
  ...
  if (options.mode !== 'gitlab') {
    return {
      ok: false,
      error: 'Website Agent V3 supports gitlab mode only; use legacy agent for static sites.',
      ownerMessage: 'This edit type requires the full site builder. Please try a simpler request.',
      strategy: 'agent_loop',
      tier: 'L3',
      confidence: 'low',
    };
